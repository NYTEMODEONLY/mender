...
case "${1:-start}" in
  ...
  setup)
    if [ -z "$BOOT_PY" ]; then
      echo "No Python runtime found for Mender setup."
      exit 1
    fi
    shift || true
    "$BOOT_PY" "$MENDER_ROOT/support/mender_boot.py" setup "$@"
    exit $?
    ;;
  audit)
    if [ -z "$BOOT_PY" ]; then
  ...
  start|"")
    ;;
  *)
    echo "Usage: $0 [start|setup|doctor|doctor-json|ready|set-key|audit|update|update-hermes]"
    exit 2
    ;;
esac
