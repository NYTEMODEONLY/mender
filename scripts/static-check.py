...
def main() -> int:
    ...
    for path in ("bootstrap.sh", "mender.sh", "update-hermes.sh"):
        text = read(path)
        require("UV_PYTHON_INSTALL_DIR" in text and "mender-install-home" in text, f"{path} must keep uv Python in a stable host scratch path")
        require("hermes_runtime_usable" in text or path != "mender.sh", "mender.sh must reinstall stale host runtimes")
    for path, text in (("Start-Mender.ps1", start_ps), ("bootstrap.ps1", bootstrap_ps)):
        require("UV_PYTHON_INSTALL_DIR" in text and "mender-install-home" in text, f"{path} must keep uv Python in a stable host scratch path")
    require("Test-HermesRuntime" in start_ps, "Start-Mender.ps1 must reinstall stale host runtimes")
    ...
    require("hermes_python_usable" in boot_py, "Mender readiness must verify Hermes Python is executable")
    require("hermes_python_on_drive" in boot_py, "Mender readiness must report whether Hermes Python resolves on the drive")
